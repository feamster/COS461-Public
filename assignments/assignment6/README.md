...
Running `sudo python start_net.py`from your vagrant VM terminal will setup the following network in mininet:
...
The start_net.py script opens a mininet command line interface (CLI) once the network is up and running. You can use mininet CLI commands to get network information and interact with the hosts and switches on your network if needed. Typing `help` into the mininet CLI will give you a list of available commands.  

**Note:** To gracefully end the experiment, you should type `quit` on the mininet CLI. That takes care of cleaning up mininet and all the processes it has started. If exited with an exception, you should run the following commands:
```
sudo mn -c
...
The middlebox in the simulated network topology runs the python script  `mb.py`. This script uses a python library called Scapy to sniff packets from the two interfaces of the middlebox, applies the `handle_packet()` method to each packet, and sends the packet out of the correct interface.  You have to add code to the sections of the `mb.py`  marked `TODO` in order to implement the DNS reflection attack detection and mitigation strategy described in the "Background" section above. Do not modify any existing code in `mb.py` because it is necessary for correct packet routing.
...
Implement rate limiting by randomly dropping DNS response packets headed to the victim host with high probability `P`. Use the `random()` function to generate random numbers, documentation here: https://docs.python.org/2/library/random.html.  The value `P` is up to you.  You will asked to justify the value you chose in the last part of the assignment.  Drop packets by returning from the  within the `handle_packet()` method before the `sendp()` call.  
...
**Debugging Tips:**
1. To debug your code in `mb.py`, you can print informative messages to stderr using `printf(message)`.  These messages will be directed to the `mb-log.txt` file.  Please remove these prints before submitting.  Stderr for `mb.py` will automatically print to `mb-err.txt`.
...
3. Remember to run the following commands if your program crashes, mininet exists with an exception, or you get "Please shut down the controller which is running" errors:
```
sudo mn -c
pgrep -f python | xargs kill -9
```
...
## Submission & Grading
Submit your `mb.py`, `questions.txt`, and `DNS_response_rates.png` files to CS DropBox here: https://dropbox.cs.princeton.edu/COS461_S2017/Assignment_6_-_DNS_Reflection.
